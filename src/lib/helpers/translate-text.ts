...
export const translateText = async (
  ...
  try {
    ...
    // Use user-provided key if available, otherwise fall back to built-in key rotation
    ...
    // Fall back to built-in key rotation
    const keys = BUILTIN_API_KEYS[provider];
    if (!keys || keys.length === 0) {
      throw new Error(
        `No API keys configured for provider "${provider}". Set it in Tools > API Keys.`
      );
    }

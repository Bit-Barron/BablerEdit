...
export const useConfigureLang = () => {
  const { parsedProject, setParsedProject, projectSnapshot } =
    useProjectStore();
  ...
  const addPathTolanguage = async (locale: string) => {
    const openFile = await open({
      multiple: false,
      directory: false,
      filters: [{ extensions: ["json"], name: "Translation JSON" }],
    });
    ...
    const checkIfFileAlreadyExist = parsedProject.translation_packages.some(
      (t) => {
  ...
  const handleDelete = async (locale: string) => {
    ...
    const filterLocale = parsedProject.languages.filter(
      (locale) => locale.code !== splitLocale,
    );
    const filterTranslationPackages =
      parsedProject.translation_packages[0].translation_urls.filter(
  ...
  const handleFilter = async () => {
    const sourceProject =
      projectSnapshot && Object.keys(projectSnapshot).length > 0
        ? projectSnapshot
        : parsedProject;

https://ui.aceternity.com/components/3d-card-effect
https://ui.aceternity.com/components/text-generate-effect
https://ui.aceternity.com/components/floating-dock#
https://ui.aceternity.com/components/file-upload
https://www.diceui.com/docs/components/file-upload
https://ui.eindev.ir/docs/components/glass-notification
https://www.eldoraui.site/docs/components/dock-text


https://magicui.design/docs/components/particles

C:\Users\barron\AppData\Roaming\com.barron.bableredit\settings.json

/// DONNN
<!-- https://ui.tripled.work/components/glass-wallet-card -->
<!-- https://www.ui-layouts.com/components/range-slider -->

<!-- https://ui.tripled.work/components/wizard-form -->
<!-- https://ui.tripled.work/components/ai-unlock-animation -->
<!-- https://www.scrollxui.dev/docs/components/hyperlink -->
<!-- https://motion-primitives.com/docsmponents/hyperlink -->
<!-- https://motion-primitives.com/docs/border-trail -->
<!-- https://magicui.design/docs/components/border-beam -->
<!-- https://magicui.design/docs/components/particles -->
<!-- https://magicui.design/docs/components/animated-gradient-text -->
<!-- https://magicui.design/docs/components/warp-background -->
<!-- https://efferd.com/blocks/cta -->
<!-- https://ui.aceternity.com/components/container-cover -->
<!-- https://www.ui-layouts.com/components/sparkles-title -->
<!-- https://ui.tripled.work/components/dynamic-spotlight-cta -->
<!-- https://ui.tripled.work/components/floating-gradient -->
<!-- https://ui.spectrumhq.in/docs/multistepform -->
<!-- https://www.scrollxui.dev/docs/components/decorated-text -->
<!-- https://www.scrollxui.dev/docs/components/not-found  -->
<!-- https://reactbits.dev/text-animations/variable-proximity -->
<!-- https://reactbits.dev/animations/star-border -->
<!-- https://reactbits.dev/components/gooey-nav -->
<!-- https://chanhdai.com/components/testimonials-marquee -->
<!-- https://magicui.design/docs/components/marquee -->
<!-- https://magicui.design/docs/components/animated-beam -->
<!-- https://magicui.design/docs/components/light-rays -->
<!-- https://lucide-animated.com/ -->
<!-- https://intentui.com/blocks navbar -->
<!-- https://kokonutui.com/docs/components/beams-background -->
<!-- https://www.diceui.com/docs/components/marquee -->
<!-- https://aliimam.in/blocks/header -->
<!-- https://ui.aceternity.com/components/spotlight-new -->
<!-- https://ui.aceternity.com/components/infinite-moving-cards -->
<!-- https://ui.aceternity.com/components/hover-border-gradient -->

coding.global

<!-- https://www.animbits.dev/docs/transitions/view-toggle -->
<!-- https://ui.spectrumhq.in/docs/imagepreview -->
<!-- https://shadcn-editor.vercel.app/ -->
<!-- https://platejs.org/ -->
<!-- https://prosekit.dev/ -->
<!-- https://ui.8starlabs.com/docs/components/status-indicator -->
